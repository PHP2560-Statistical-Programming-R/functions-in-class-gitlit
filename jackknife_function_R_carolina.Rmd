...
It is because this is related to the t-test, which is always n-1 degrees of freedom
...
mean.jackknife <- function(a_vector) {
  ...
  avg <- vector(length=n)
  for (i in 1:n) {
    avg[i] <- mean(a_vector[-i])
  }
  approx.var <- var(avg) * ((n-1)^2) / n
...
gamma.jackknife <- function(a_vector) {
  ...
  a <- numeric(n)
  s <- numeric(n)
  estimator <- vector(length=n)
  for (i in 1:n) {
    a[i] <- gamma.est(a_vector[-i])[1]
    s[i] <- gamma.est(a_vector[-i])[2]
   }
  approx.var.a <- var(a) * ((n-1)^2) / n
  approx.var.s <- var(s) * ((n-1)^2) / n
  jackknife.stderr <- c(sqrt(approx.var.a), sqrt(approx.var.s))
  # Put your function in here
  return(jackknife.stderr)
}
```
...
jackknife.lm <- function(df, formula, p) {
  n <- nrow(df)
  coeff <- data.frame(matrix(NA, ncol=p+1, nrow=n))
  for (i in 1:n) {
   coeff[i,] <- (lm(formula, df[-i,]))$coefficients
  }
  approx.var <- map_dbl(coeff, var) * ((n-1)^2) / n
  jackknife.stderr <- sqrt(approx.var)
  return(jackknife.stderr)
}
```
