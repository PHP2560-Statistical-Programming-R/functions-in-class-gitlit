...
jackknife.lm <- function(df,formula,p) { #  p is the number of parameters besides the intercept
  n<-nrow(df)
  linear<-matrix(NA,nrow=nrow(df),ncol=p+1)
  
  for ( i in 1:nrow(df)){
  linear[i,]<-lm(formula,data=df[-i,])$coefficients  # formula represents the x and y variables
 
  }
  linear<-as.data.frame(linear)
  var_estimate<-map_dbl(linear,var) 
  var_estimate_linear<-var_estimate*(n-1)^2/(n)
  
  jackknife.stderr<-sqrt(var_estimate_linear)
...
jackknife <- function(estimator,the_data) {  
    ...
    n <- length(the_data)    # Define n to be length or number of rows here
  } else{
    n<-nrow(the_data)
  }
    omit_and_est <- function(omit) {
