...
n<-length(a_vector)  
iterated.mean<-rep(0,n)
for (i in 1:n) {
iterated.mean[i]=mean(a_vector[-i])
}

iterated.var<-var(iterated.mean)  
approx.var<-iterated.var*(n-1)^2/n
jackknife.stderr<- sqrt(approx.var)
...
gamma<-matrix(rep(0,2*n),nrow=n,ncol=2)
for (i in 1:n) {
gamma[i,1]<-gamma.est(a_vector[-i])[[1]]
gamma[i,2]<-gamma.est(a_vector[-i])[[2]]
}
jack.a<-gamma[,1]
jack.s<-gamma[,2]

var.a <- var(jack.a) * (((n-1)^2) / n)
var.s <- var(jack.s) * (((n-1)^2) / n)

jackknife.stderrs <- c(a=sqrt(var.a),s=sqrt(var.s))
...
jackknife.lm <- function(df,formula,p) {
  ...
  for(i in 1:n) {
    jack.coeff <- lm(as.formula(formula), data=df[-i,])$coefficients
    y[,i]<-jack.coeff
}
var.coeff<-apply(y, 1, var)
jack.stderr<-sqrt(var.coeff* (((n-1)^2) / n))
  names(jack.stderr) = names(jack.coeff)
  return(jack.stderr)
}
...
scale_and_sqrt_vars <- function(jackknife.ests,n) {
  estimate.var <- apply(jackknife.ests,1,var)
jack.stderr <- sqrt(((n-1)^2/n)* estimate.var)
return(jack.stderr)
}
```
