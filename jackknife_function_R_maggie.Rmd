...
mean.jackknife <- function(a_vector) {
  ...
  means <- numeric(n)
  for(i in 1:n){
    new_vector <- a_vector[-i]
    means[i] <- mean(new_vector)
  }
  
  jackknife.var <- var(means) * (((n-1)^2) / n)
...
gamma.jackknife <- function(a_vector) {
  ...
  for(i in 1:n){
    new_vector <- a_vector[-i]
    a[i] <- gamma.est(new_vector)[1]
    s[i] <- gamma.est(new_vector)[2]
  }
  jackknife.var.a <- var(a) * (((n-1)^2) / n)
  jackknife.var.s <- var(s) * (((n-1)^2) / n)
  jackknife.stderrs <- c(a.se=sqrt(jackknife.var.a),s.se=sqrt(jackknife.var.s))
...
jackknife.lm <- function(df,formula,p) {
  n <- nrow(df)
  coeffs <- data.frame(matrix(NA, ncol = p + 1, nrow = n))
  names(coeffs) <- names(coefficients(lm(formula, df)))
  
  for(i in 1:n){
    new_df <- df[-i,]
    coeffs[i,] <- coefficients(lm(formula, new_df))
  }
  
  jackknife.vars <- apply(coeffs, 2, function(x) var(x) * (((n-1)^2)/n))
  jackknife.stderrs <- sqrt(jackknife.vars)
  names(jackknife.stderrs) = names(coeffs)
  return(jackknife.stderrs)
}
```
...
all.equal(jackknife(estimator=est.coefs,the_data=data),
          jackknife.lm(df=data,formula="output~input",p=1), check.names = FALSE)
```
...
```{r, eval=FALSE}
  data_size <- function(the_data) {
    ...
    ```
- Write a function that calculates the variance of all the estimates and returns the standard error
```{r, eval=FALSE}
